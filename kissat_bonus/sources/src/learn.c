...
kissat_learn_clause (kissat * solver)
{
  ...
  // solver->cur_bump_bounus = pow(solver->averages->slow_glue.value/glue,2);
  // solver->cur_bump_bounus = pow(0.25, (double)glue - solver->averages->slow_glue.value); 
  ++solver->clause_sum;
  solver->avg_lbd = (solver->avg_lbd * (solver->clause_sum-1) + (double) glue) / solver->clause_sum;
  solver->cur_bump_bounus = pow(pow(3.0, 1.0/(1.0 - solver->avg_lbd)), (double)glue - solver->avg_lbd);
  if (solver->cur_bump_bounus < 0.2) solver->cur_bump_bounus = 0.2;
  //solver->cur_bump_bounus = pow(pow(2.0, 1.0/(1-solver->averages->slow_glue.value)), (double)glue - solver->averages->slow_glue.value);
  if (!solver->probing)
    {
